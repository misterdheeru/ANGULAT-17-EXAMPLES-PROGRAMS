...
export class EmployeeCRUDService {
  ...
  constructor(private httpclient : HttpClient)
   ...
   UpdateEmployees(stdID:any,stdObj:any)
   {
    let Obsorable = this.httpclient.put(`${this.url} `+ stdID , stdObj)
    return Obsorable;
   }

   getSingleEmploeee(stdID:any)
   {
    let Obsorable =  this.httpclient.get(`${this.url} ${stdID}`)
    return Obsorable;
